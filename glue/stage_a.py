...
def log_distinct_column_values(frame: DynamicFrame, column_name: str) -> None:
    """
    Logs all distinct values of a given column from a DynamicFrame.
    """
    df = frame.toDF()
    ...
    logger.info(f"Found {len(distinct_values)} distinct values in column '{column_name}'")
    logger.info(f"Distinct values: {distinct_values}")
...
try:
    ...
    # 🔍 Log distinct values for dpto_pjfs in each dataset
    log_distinct_column_values(delitos_arequipa, "dpto_pjfs")
    log_distinct_column_values(delitos_junin, "dpto_pjfs")
    ...
    # 🔍 Log distinct values of dpto_pjfs
    log_distinct_column_values(combined_delitos, "dpto_pjfs")
